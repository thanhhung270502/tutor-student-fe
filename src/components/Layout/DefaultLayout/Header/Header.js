...
const Guest = () => {
    ...
    const link = ["/signup", "/classlist", "/aboutus"]
    return (
        <React.Fragment>
            {
                guest.map((v, index) =>
                    <li key={index} className={`nav-item ${clsx(style.headerItem)}`}>
                        <Link to={link[index]} className={`nav-link ${clsx(style.link)}`}>{v}</Link>
                    </li>
                )
            }
            <li className={`nav-item ${clsx(style.headerItem)}`}>
                <Link to="/login" className={`nav-link ${clsx(style.login)}`}>Đăng nhập</Link>
            </li>
            <li className={`nav-item ${clsx(style.headerItem)}`}>
                <Link to="/signup" className={`nav-link ${clsx(style.signup)}`}>Đăng ký</Link>
            </li>
        </React.Fragment>
    )
}
        ...
        <React.Fragment>
            ...
            <li className={`nav-item dropdown ${style.dropdown}`}>
                ...
                <ul className={`dropdown-menu ${style.dropdownContent}`}>
                    <li className={`nav-item`}><Link className={`nav-link dropdown-item ${clsx(style.dropdownItem)}`} to="#">Thông tin cá nhân</Link></li>
                    <li className={`nav-item`}><Link className={`nav-link dropdown-item ${clsx(style.dropdownItem)}`} to="#">Xác nhận thanh toán</Link></li>
                    <li className={`nav-item`}><Link className={`nav-link dropdown-item ${clsx(style.dropdownItem)}`} to="#">Đăng xuất</Link></li>
                </ul>
            </li>
        </React.Fragment>
    );
    ...
    return (
        <React.Fragment>
            ...
            <li className={`nav-item dropdown ${style.dropdown}`}>
                ...
                <ul className={`dropdown-menu ${style.dropdownContent}`}>
                    <li className={`nav-item`}><Link className={`nav-link dropdown-item ${clsx(style.dropdownItem)}`} to="#">Thông tin cá nhân</Link></li>
                    <li className={`nav-item`}><Link className={`nav-link dropdown-item ${clsx(style.dropdownItem)}`} to="#">Thống kê</Link></li>
                    <li className={`nav-item`}><Link className={`nav-link dropdown-item ${clsx(style.dropdownItem)}`} to="#">Đăng xuất</Link></li>
                </ul>
            </li>
        </React.Fragment>
